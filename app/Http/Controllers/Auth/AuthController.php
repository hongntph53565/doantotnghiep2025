...
if (!$user) {
     return redirect()->route('register.form')->with('error', 'Vui lòng đăng ký để xem thông tin.');
        ...
        $points = $membershipCard?->points ?? 0;
        $cardType = $membershipCard?->card_type ?? 'normal';
        $cardNumber = $membershipCard?->card_number ?? 'N/A';

        // ✅ Tính còn bao nhiêu điểm để lên hạng tiếp theo
        $nextRank = null;
        $pointsNeeded = null;
        if ($cardType === 'normal') {
            $nextRank = 'silver';
        ...
        return view('Client.profile', compact(
            ...
            'totalRP',
            // 'cardLevel',
            'monthlySpendingForSelectedMonth',
            'typeFilter',
            'monthFilter',
            'monthlySpending',
            'provinces',
