...
class ZalopayApiController extends Controller
{
    protected $zalopay;
    ...
    public function createLink(Request $request)
    {
        ...
        if (!$amount || !$description) {
            return response()->json([
                'success' => false,
                'message' => 'Thiếu amount hoặc description'
            ], 400);
        }

        $response = $this->zalopay->createPaymentLink($amount, $description);
        ...
        if (isset($response['return_code']) && $response['return_code'] == '1') {
            ...
            return response()->json([
                'success' => false,
                'message' => $response['desc'] ?? 'Không rõ lý do'
            ], 500);
        }
    }

    // API: Xử lý return từ Zalopay (redirect user về sau thanh toán)
