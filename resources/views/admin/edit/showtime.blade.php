...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const rapChieuSelect = document.getElementById("rapChieu");
    const roomSelect = document.getElementById("room");

    // Lọc rạp theo thành phố
    function filterRapChieu() {
        const selectedCity = districtSelect.value;
        const options = rapChieuSelect.querySelectorAll("option");

        let hasSelected = false;
        options.forEach(option => {
            if (!option.hasAttribute("district-data")) return;
            if (option.getAttribute("district-data") === selectedCity) {
                option.style.display = "block";
                if (!hasSelected && option.selected) hasSelected = true;
            } else {
        ...
        if (!hasSelected) rapChieuSelect.value = "";
        filterRooms(); // reset phòng khi đổi thành phố
    }

    // Lọc phòng theo rạp
    function filterRooms() {
        const selectedCinema = rapChieuSelect.value;
        const options = roomSelect.querySelectorAll("option");

        let hasSelected = false;
        options.forEach(option => {
            if (!option.hasAttribute("cinema-data")) return;
            if (option.getAttribute("cinema-data") === selectedCinema) {
                option.style.display = "block";
                if (!hasSelected && option.selected) hasSelected = true;
            } else {
                option.style.display = "none";
        ...
        if (!hasSelected) roomSelect.value = "";
    }

    // Chạy khi load trang (edit)
    filterRapChieu();
    filterRooms();

    // Gắn event
    districtSelect.addEventListener("change", filterRapChieu);
    rapChieuSelect.addEventListener("change", filterRooms);
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function calcEndTime() {
        ...
        const duration = parseInt(movieOption?.dataset.duration || 0); // phút
        ...
        minutes = minutes % 60;
        hours = hours % 24; // vòng lại 0-23 nếu qua ngày

        // format lại HH:mm
        const endHour = String(hours).padStart(2, "0");
    ...
    // Event
    movieSelect.addEventListener("change", calcEndTime);
    startTimeInput.addEventListener("input", calcEndTime);

    // Tự tính khi load trang edit
    calcEndTime();
});
</script>
@endpush
